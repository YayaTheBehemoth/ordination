...
public class DataService
{
    ...
    public PN OpretPN(int patientId, int laegemiddelId, double antal, DateTime startDato, DateTime slutDato) {
       var lm = db.Laegemiddler.FirstOrDefault(l => l.LaegemiddelId == laegemiddelId);
        PN newpn = new PN {startDen = startDato, laegemiddel = lm,slutDen = slutDato, antalEnheder = antal};
    ...
    double antalMorgen, double antalMiddag, double antalAften, double antalNat, 
        ...
        DagligFast newFast = new DagligFast{startDen = startDato, slutDen = slutDato,laegemiddel = lm,MorgenDosis = MD, MiddagDosis = MiD,AftenDosis = AD, NatDosis = ND};
        var patient = db.Patienter.FirstOrDefault(p => p.PatientId == patientId);
        patient.ordinationer.Add(newFast);
        db.SaveChanges();
    ...
    public DagligSkæv OpretDagligSkaev(int patientId, int laegemiddelId, Dosis[] doser, DateTime startDato, DateTime slutDato) {
        var lm = db.Laegemiddler.FirstOrDefault(l => l.LaegemiddelId == laegemiddelId);
        DagligSkæv newskæv = new DagligSkæv {startDen = startDato, slutDen = slutDato, laegemiddel = lm, doser = doser.ToList()};
        var patient = db.Patienter.FirstOrDefault(p => p.PatientId == patientId);
        patient.ordinationer.Add(newskæv);
        db.SaveChanges();
        return newskæv;
    }

    public string AnvendOrdination(int id, Dato dato) {
        var ordtocheck = db.PNs.FirstOrDefault(o => o.OrdinationId == id);
        if (ordtocheck.givDosis(dato) == true){
           //dato.dato >= ordtocheck.startDen && dato.dato <= ordtocheck.slutDen
           ordtocheck.dates.Add(dato);
           db.SaveChanges();
            return "Din anmodning er blevet godkent, ordinationen kan anvendes";
        }else{
            return "Din anmodning er blevet afvist, da du har forsøgt at anvende ordinationen udenfor dennes gyldighedsperiode";
        }
	...
	public double GetAnbefaletDosisPerDøgn(int patientId, int laegemiddelId) {
        var lm = db.Laegemiddler.FirstOrDefault(l => l.LaegemiddelId == laegemiddelId);
        var patient = db.Patienter.FirstOrDefault(p => p.PatientId == patientId);
        if(patient.vaegt < 25){return lm.enhedPrKgPrDoegnLet;}
        else if(patient.vaegt > 120){return lm.enhedPrKgPrDoegnTung;}
        else{return lm.enhedPrKgPrDoegnNormal;}
